...
Free-text fields go through `TyrionWeb::Presenter.markdown_lite` — HTML-escape first, then a minimal bold/italic/inline-code/paragraph pass (code spans are pulled out before the emphasis passes so `` `*.rb` `` can't have its asterisk read as emphasis, then restored). It returns a plain `String`; views wrap it with Phlex's own `safe(...)` before `raw(...)`, so the presenter carries no Phlex dependency. This is a local, minimal helper — `discoveries-markdown-rendering` (a sibling story) owns building the real shared markdown helper once dispatched, at which point this one is expected to be replaced, not extended.
