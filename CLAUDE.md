...
**Key files:**
- **`web/app.rb`** — Sinatra routes. One route per view + `GET /api/poll` for live monitoring. Mutations use PRG (Post/Redirect/Get) with `session[:flash]`. `with_flash` helper in `helpers` block DRYs up all POST error handling; `with_epic_flash` and `with_discovery_flash` are parameterized variants that redirect back to the epic/discovery instead of `with_flash`'s hardcoded `/`. `safe_return_to` guards the discovery mutation routes' `params[:return_to]` against open redirect (protocol-relative `//`, backslash-normalized `/\`, full URLs) before it's ever passed to `redirect`.
- **`web/views/discovery_show.rb`** — `Views::DiscoveryShow` Phlex component, the `GET /discoveries/:id` page. Full record (headline, question, finding/confidence/recommendation rendered as three independently-gated lines — none hidden behind another field's presence) plus real mutation forms: Defer (optional reason), Promote (needs a real epic picker — `promote_discovery_to_story` has no safe default epic), Edit headline. `epic:` (the discovery's true epic, possibly nil) is kept separate from `sidebar_epic:` (falls back to the resolved active epic for Layout's sidebar) so the page's own content never claims an epic the discovery doesn't actually have.
...
**Discoveries index poll (`GET /api/discoveries_poll?project=<slug>`)** — the third poller, and deliberately the story poller's pattern (reload-on-token-change), not ambient's DOM-patch: the Discoveries index is a full list page with no mid-read state a reload would blank. `TyrionWeb::Data.discoveries_token` fingerprints everything the page renders — the active spike's id/question/hypothesis/exit_criteria, every findings_ready/mark id + its glance content, and (unlike `ambient_token`) each row's `aged?` *boolean* — a threshold crossing flips a boolean exactly once, so it doesn't churn the token every tick the way raw `created_at` would (same reasoning ambient_token documents, applied to a page where reload makes recomputing the boolean per-tick unnecessary). The page seeds the badge's `data-token` from `discoveries_token` at render time (same reason `/ambient` seeds its token) so the poll JS has no null-sentinel bootstrap branch and a stray falsy token from a 404 response can't latch and permanently disable reloading. The badge reuses `active_story.rb`'s `#poll-badge`/`#poll-dot` ids (each route has its own DOM, so no collision) to pick up `shared.css`'s fade-in/pulse animation for free instead of shipping a fourth copy of it. Scoped to the `/discoveries` index only — the per-discovery show page (`/discoveries/:id`) has no poller.

**Ambient pane (`GET /ambient` + `GET /api/ambient_poll?project=<slug>`)** — `web/views/ambient.rb` renders a chrome-free glance surface (newest 10 open marks + a findings_ready line, `mark_limit:` on `load_ambient_view`) for a narrow browser pane beside a terminal. Each mark is a compact row (headline/question + age) that expands in place on click — progressive disclosure, no navigation for the common case — revealing the full text, a one-click `Defer` form, and an `Open full view →` link to `/discoveries/:id`. Its poller is deliberately *not* the story poller: 60s instead of 30s, and it patches the DOM in place instead of reloading (a reload would blank a pane someone is glancing at) — an expand/collapse repaint always collapses back to compact (no expand-state carried across a real content change), an accepted gap, not a bug. The payload is `{token, marks:[{id,text,headline,question,created_at}], findings_ready_count}` — `text` is the glance-surface fallback (`Output.discovery_glance_text`), `headline`/`question` ride along separately so the expanded state has the full record to show. `Views::Ambient#render_js` always repaints *both* the marks list and the findings_ready line together so the two can't disagree. `TyrionWeb::Data.ambient_token` fingerprints mark ids + headline + question text + the count: aging is a pure function of `created_at` and wall-clock time, so folding it into the token would churn it every tick — instead the JS recomputes each mark's aged class from `data-created-at` on *every* tick, token change or not, which is the only way a mark crossing the 14-day threshold ever turns amber (rendered as a "· stale" text label, not color alone). `AGING_DAYS`/`TRUNCATE_AT`/`POLL_INTERVAL_MS` are interpolated into the JS from the Ruby constants so the repaint can't drift from the first render. Project resolution goes through the same `load_ambient_view` the page render uses (unknown slug falls back to the active project) — a stricter lookup in the poller would blank a pane that is rendering fine. 404 is reserved for "no project resolved at all," and even then the body is the same-shaped empty payload, so the page needs no error branch. The inline Defer form redirects back to the ambient pane via `params[:return_to]` (validated by `safe_return_to`); `Views::Ambient#render_flash` reads `session[:flash]` so success and failure are both visible (`"Error:"` prefix distinguishes them) — without it, a failed defer (e.g. double-defer) looked identical to a successful one.

**Sidebar epic gating** — `Views::Layout#render_sidebar` and `TyrionWeb::Data.load_sidebar_data(project, epic)` gate on `project` alone, not `project && epic`. An epic-less project (e.g. a pure-spike project with no epics) still gets the project name and the discovery strip in the sidebar; only the epic-scoped "Stories · `<slug>`" section is conditionally omitted when `epic` is nil. The `"No active project"` fallback fires only when `project` itself is nil. `disc[:ready_count]`/`disc[:mark_count]` are coerced with `.to_i` before the `> 0` checks specifically because this path is now reachable with a bare `{}` `disc_summary` (some callers, including specs, don't always pass the full `{spike:, ready_count:, mark_count:}` shape) — guard against `NoMethodError` on `nil > 0` rather than assuming every caller passes the complete shape.
...
**Roadmap containment tree** — `TyrionWeb::Presenter.epic_state` derives a display state from an epic's own stories/status plus two graph-derived fields the epic hash must already carry: `'unmet'` (from `Store#unmet_prereqs`) and `'child_stats'` (from `Store#epic_seal_stats` — nil for a leaf, `{done:,total:}` sealed-descendant-epics counts for a container). `:container` fires only when the epic has descendants *and* zero stories of its own; `:waiting` fires only when the epic's own status is `active` and it has an unmet prerequisite. Precedence is explicit and load-bearing: `:sealed` is checked before `:container` (a sealed empty container still reads sealed, not container), and `:active`/`:cold` are checked before `:waiting` (live in-progress work still reads active even if a prerequisite regressed after the story started). `Data.load_roadmap_view` decorates every epic with `'unmet'`/`'child_stats'` from a single `epic_graph` snapshot (also returned as `graph:` in the view hash — `TyrionWeb::Data::EMPTY_EPIC_GRAPH` is the empty-safe shape when no project resolves) and adds no per-epic queries doing it. `Store.epic_tree_order(section_epics, graph)` is a class method (not an instance method like its `epic_ancestors`/`epic_descendants`/`unmet_prereqs` siblings) specifically so `Views::RoadmapView` can call it directly without holding a `Store` instance — it's the one shared depth-first tree walk both `tyrion epic list`/`epic show`/`project show` (CLI) and the Roadmap view (web) use, so there is exactly one containment-tree traversal in the codebase, not two. The view still owns root *order* (attention weight, `RoadmapView::ATTENTION_WEIGHT`) since that's a rendering concern the CLI doesn't share: it walks the tree once in the epics' natural (`created_at`) order, then reorders whole root-subtrees as units by the root's attention weight (`slice_when` on depth-zero boundaries, then `sort_by` the chunks) — children never move independently of their parent. The archived section renders through the identical `Store.epic_tree_order` call, so an archived parent's archived children nest exactly like the active tree, with a "(parent archived)" note reused from the CLI's own `epic_tree_order` semantics for a child whose real parent lives outside the current section.
...
**Global View discovery status** — `TyrionWeb::Data.load_global_view` merges `load_discovery_summary(project['id'])` into every project card so a project with zero epics but real discovery activity (an active spike, findings ready to promote, or open marks) no longer reads as `:idle` — the root cause of a spike-only project (all SDRD work, no epics) showing as fully idle on the command center right after closing a real spike queue. Precedence is deliberate and load-bearing: story activity of any kind (`:active`/`:stale`/`:done`) is checked before discovery activity, and the new `:discovery` branch only fires when the project's total story count is zero — a project with pending stories AND open marks still reads `:idle`, because the story lane stays the honest headline and the discovery strip already surfaces the rest. `TyrionWeb::Presenter.discovery_activity?`/`discovery_summary_text` both take the same `{spike:, ready_count:, mark_count:}` shape `load_discovery_summary` returns, so the boolean gate and the one-line render text (`"N ready to promote · N marks"`, shown on the card in place of "No story in progress") can't drift apart.

### Discovery layer

`discoveries` table sits directly under `projects` (not epics). Two entry modes:
- `tyrion mark "desc" [--headline "…"]` — zero-friction bookmark, status=`mark`
- `tyrion discover` — 30-second organic capture, status=`findings_ready`
- `tyrion discover <disc-id> --finding "…" [--question "…"] [--headline "…"]` — non-interactive upgrade of an existing `mark` to `findings_ready`
...
**Headline** — `discoveries.headline` is a short, agent-authored, human-facing line, distinct from `question` (the raw thing noticed, which can be as long as it needs to be). Every glance surface (ambient pane, `tyrion status`'s DISCOVERIES lane, `tyrion discovery list`) renders `headline` in preference to a truncated `question` — `Output.discovery_glance_text(disc)` is the single fallback rule (`headline` if set, else `question`) shared by CLI and web, so they can't drift. Set/update it standalone with `tyrion discovery headline <disc-id> "<text>"`. Nullable, never backfilled on pre-existing rows — same reasoning as `source_story_id`: guessing a headline for old prose would be a fabricated claim, not a fact.
...
**Blocked stories** — `blocked` is a first-class story status (alongside `pending|in_progress|done|abandoned`). Use `tyrion block <slug> "reason" [--discovery disc-NNN]` / `tyrion unblock <slug>`. Blocking stores `blocked_on TEXT` (human reason) and optionally `blocked_on_discovery TEXT` (linked disc-id). `tyrion start` refuses a blocked story with the reason and the unblock command. `tyrion status` renders a distinct `BLOCKED` lane below the story list; if the linked discovery has resolved (`promoted_to_story|deferred|invalidated`), the lane shows `[disc-NNN resolved → unblock?]`. `tyrion block` also accepts a `done` story (post-done-integrity) — e.g. a coordinator's independent adversarial-review pass finding a real bug after a builder claimed done. `Store#block_story` stashes `pre_block_status='in_progress'` (not `'done'`) for that case, carries the done row's `completed_by` forward as `pre_block_claimed_by` (a done row's own `claimed_by` is already NULL — `complete_story` nulled it), and clears `completed_at`/`completed_by` in the same UPDATE so a blocked row never claims to still be done. `Store#unblock_story` carries its own defensive coercion of the same 'done'→'in_progress' rule, so `tyrion unblock` — with or without `--resume` — can never silently re-seal a done story unverified; it always re-enters as `in_progress`, reclaimed under the lane that originally closed it.

**Reopen a done story** — `tyrion reopen <slug> "reason"` (`Store#reopen_story` + `cmd_reopen`) is the other half of post-done-integrity: it moves a `done` story back to `in_progress` for rework, without touching any criterion (`tyrion uncheck <slug> <position>` is the separate, existing tool for invalidating one criterion). Refuses with a clear message unless the story's current status is `done`. Clears `completed_at`/`completed_by` and re-claims the story under the reopening lane (mirrors `start_story`) — a reopened story left unclaimed-`in_progress` would read as a `tyrion violations` protocol violation and couldn't be claimed back via `tyrion start` (which requires `pending`). Records a `recovery` note (reason + prior status) and reopens the epic via the same `reopen_epic_if_done!` helper `block_story` uses, so a sealed epic un-seals if this was its last done story.
...
**Dark-factory gate-completeness guard** — a `dark_factory`-mode epic (`epic['mode']`, `Output.dark_factory?`) unions `DARK_FACTORY_REQUIRED_GATES` (`%w[pre-push uat]`) into `cmd_done`'s `required_gates` automatically, even with no `--require-gates` flag on the invocation — an explicit `--require-gates=<names>` on such an epic adds to that pair (`|`, never `+`/replace). This is separate from and additional to the failing-gate check above: `--force` still only bypasses a *failing* gate result, never *missing* coverage — a dark_factory story with an absent `pre-push` or `uat` gate note cannot be force-closed through `tyrion done`. The refusal message names the mode and the escape hatch (`tyrion epic mode <slug> shape`) rather than blaming a flag the caller never typed. `tyrion audit [--epic <slug>]` is the read-only backstop for what this can't retroactively fix — a story that closed before the guard existed, or whose epic's mode changed to `dark_factory` after the story was already done; it scans done stories per dark_factory epic via the same `recorded_gate_names` helper `cmd_done` uses, printing one `<slug> (<epic-slug>): missing <gates>` line per gap (a clean run prints a single "No gaps found" line). `--epic <slug>` naming a non-dark_factory epic says so plainly rather than reporting a misleading "no gaps".
...
**Discovery deletion** — `tyrion discovery delete <disc-id>` is the one truly irreversible discovery verb; everything else (`defer`, `discover <id>` upgrade, `spike done`) is a status flip that keeps the row findable. `Store#delete_discovery` refuses whenever something else still points at the row: a `promoted_to_story` discovery (naming the story slug in the error — checking `status`, not `story_id`, since `status` is the stricter guarantee that a linked story row still exists), or a story currently `blocked_on_discovery` that id (naming the blocked story's slug and the `tyrion unblock` command to clear it) — `blocked_on_discovery` has no FK/cascade, unlike `born_from_discovery`'s `ON DELETE SET NULL`, so an unguarded delete would strand the story pointing at nothing. Same project-scope boundary as `cmd_discover_upgrade`: a disc-id in another project reports plain "not found", and since delete is irreversible, leaking a cross-project id is worse here than on upgrade. The CLI echoes the full row (via the same `print_discovery_detail` renderer `discovery show` uses) rather than `Output.discovery_glance_text` — glance text is deliberately lossy (headline-or-truncated-question) and a permanent delete is the one place that's the wrong tradeoff.
...
**Discovery verdict** — `discoveries.verdict` (nullable TEXT, `CHECK verdict IS NULL OR verdict IN (...)`) is orthogonal to `status`: status tracks lifecycle (`active_spike` → `findings_ready` → `promoted_to_story`/`deferred`/`invalidated`), verdict scores whether the hypothesis held up — one of `Store::VERDICTS` (`confirmed`/`falsified`/`falsified_alternative`/`partial`). It is set **only** by `tyrion spike done --verdict <value>`, on the `active_spike` → `findings_ready` transition, via `Store#close_spike` — a plain assignment, not the `COALESCE` pattern `origin` uses above, because there is no prior verdict on that row to preserve. Omitting `--verdict` leaves it `nil` (unscored), never defaulted to `confirmed`. `Tyrion::Output.verdict_label` (`verdict.tr('_', ' ')`) is the single source of the humanized wording, shared by `tyrion discovery show` and `TyrionWeb::Presenter.verdict_tag` (web Discoveries list + per-discovery show page) so the two can't spell `falsified_alternative` differently. Rendering a *missing* verdict as "(unscored)" is gated to `Store::SCOREABLE_STATUSES` (`findings_ready`, `promoted_to_story`) — statuses guaranteed to have passed through `close_spike` — rather than a denylist on the current status, because a `mark` can be deferred without ever reaching `close_spike`, landing on `deferred` same as a scored-then-deferred finding; an *actual* verdict value always renders regardless of current status, only a missing one is gated. Currently spike-close-only: `tyrion discover` and `tyrion discovery list`/`tyrion status` don't set or surface it (tracked as disc-064).
...
`discoveries.parent_spike_id` is the sibling column for spikes, but scoped differently: `source_story_id` is lane-scoped (resolved through `prime_story_for`, so another lane's in-progress story never attributes a mark to it), while `parent_spike_id` is project-scoped — `tyrion mark` looks up `Store#active_spike_for(project_id)`, and a spike is project-global by construction (`idx_one_active_spike_per_project` allows at most one `active_spike` row per project, full stop, no lane dimension). No new flag; set once at `create_discovery` time and never touched by any later status transition (including the spike's own `active_spike → findings_ready` close), same no-backfill-on-legacy-rows contract as `source_story_id`. Read it as recording *co-occurrence*, not causation: in a parallel-lane session, a mark filed on lane-5 while lane-2 has a spike in flight gets stamped with lane-2's spike id, because there is no lane-scoped spike to prefer over the project's one active spike.

**Discovery show page** (`GET /discoveries/:id`, `Views::DiscoveryShow`) renders every field the CLI's `tyrion discovery show` prints (question/hypothesis/exit_criteria/finding/confidence/recommendation/defer_reason) plus a nested list of child marks — `TyrionWeb::Data.load_discovery_show_view`'s `child_marks:` is every `status='mark'` row whose `parent_spike_id` matches this discovery. `verdict` is not in that field list — it renders as its own header badge (`render_verdict` → `TyrionWeb::Presenter.verdict_tag`, same "nothing honest to show" nil-renders-nothing rule as the Discoveries list), so it isn't repeated as a plain labeled field. `load_discoveries_view`'s flat marks list excludes those same rows so a nested mark isn't shown twice — but the sidebar's project-wide `disc_summary` mark count (`load_discovery_summary`, shown on every page, not just this one) deliberately still counts them: that figure answers "how many open marks total," and narrowing it to match one filtered list would make it disagree with `tyrion status`/the ambient pane instead. Every card on the flat `/discoveries` index (`web/views/discoveries.rb`) links to its own `/discoveries/:id` via `.dv-card-id-link` — without it, a mark excluded from the flat list for being nested would have no page linking to where it moved.

Free-text fields go through `TyrionWeb::Presenter.markdown_lite` — HTML-escape first, then a minimal bold/italic/inline-code/bullet-list/paragraph pass. Blocks (blank-line-separated) are split BEFORE any inline processing, and a list block has its `-`/`*` marker stripped before that per-line inline pass runs — doing it in the other order (inline pass over the raw block, list-detection after) lets a bullet's own marker pair up with a later `*` on the same line and get silently swallowed as an emphasis delimiter. Code spans are pulled out before the bold/italic passes so `` `*.rb` `` can't have its asterisk read as emphasis, then restored. It returns a plain `String`; views wrap it with Phlex's own `safe(...)` before `raw(...)`, so the presenter carries no Phlex dependency. `discoveries-markdown-rendering` extended this same helper (added list support) and reused it on the Discoveries index (`web/views/discoveries.rb`'s `render_question`/`render_labeled_md`/`render_labeled_plain`) rather than building a second implementation — it is now the one shared markdown helper for both the index and the per-discovery show page.
