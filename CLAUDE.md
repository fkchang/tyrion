...
**Discovery deletion** — `tyrion discovery delete <disc-id>` is the one truly irreversible discovery verb; everything else (`defer`, `discover <id>` upgrade, `spike done`) is a status flip that keeps the row findable. `Store#delete_discovery` refuses whenever something else still points at the row: a `promoted_to_story` discovery (naming the story slug in the error — equivalent to checking `story_id` is set, since only `promote_discovery_to_story` ever writes it), or a story currently `blocked_on_discovery` that id (naming the blocked story's slug) — `blocked_on_discovery` has no FK/cascade, unlike `born_from_discovery`'s `ON DELETE SET NULL`, so an unguarded delete would strand the story pointing at nothing. Same project-scope boundary as `cmd_discover_upgrade`: a disc-id in another project reports plain "not found", and since delete is irreversible, leaking a cross-project id is worse here than on upgrade.
