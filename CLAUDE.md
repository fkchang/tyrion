...
**Epic switcher dropdown** — the topbar epic crumb is a `<select data-action="epic-switch">` (rendered by `Views::Layout#render_epic_switcher`), not a plain span. `TyrionWeb::Data.epic_switcher_epics(project)` lists every epic in the project decorated with `done`/`total` story counts and `cli_active` (whether it matches the *raw* `.tyrion/active-epic` file content via `Tyrion::Repo.active_epic`, not `resolve_active_epic`'s DB-status fallback — this is a legacy/shared-file read, same quirk noted in AGENTS.md). Fully-done epics get a trailing `✓`; the CLI-active epic gets a trailing `⚑`. A delegated `change` listener (`render_epic_switcher_js`) navigates via client-side GET to the same pathname with `epic=` merged into existing query params — never a POST, never touches `.tyrion/active-epic`. `epic_switcher:` is threaded through `load_sidebar_data`/`load_active_story_view`/`load_story_view` and all 7 routes/View classes alongside `stories:`/`disc_summary:`, following that existing threading convention.
