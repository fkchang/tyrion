...
- **Dark-factory epics can't skip the gate record.** A `dark_factory`-mode epic (`tyrion epic mode <slug>`) unions `pre-push` and `uat` into the close's required-gate coverage automatically — no `--require-gates` flag needed, and `--force` bypasses a *failing* gate but never *missing* coverage. The only escape hatch is flipping the epic to `shape` mode first, a deliberate, visible decision rather than a silent bypass. `tyrion audit [--epic <slug>]` is the read-only backstop for gaps this can't retroactively fix — a story that closed before the guard existed, or whose epic's mode changed after the fact.
