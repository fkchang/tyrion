...
5. **`tyrion check <slug> <position> "evidence"`** — mark a criterion met,
   ...
   **`tyrion uncheck <slug> <position>`** to revert if evidence turns out
   wrong.
6. **`tyrion done <slug> "summary" [--force] [--require-gates=<n1,n2>]`** —
   complete the story once all criteria are checked. `--force` overrides
   unmet-criteria protection; avoid it unless you have a documented reason
