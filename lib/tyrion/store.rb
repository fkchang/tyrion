...
module Tyrion
  ...
  class Store
    ...
    def close_spike(id, finding:, confidence:, recommendation:, origin: 'human')
      with_db do |db|
        db.transaction(:immediate) do
          ...
          t = now
          db.execute(
            "UPDATE discoveries SET status='findings_ready', finding=?, confidence=?, recommendation=?, origin=?, updated_at=? WHERE id=?",
