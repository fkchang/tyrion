...
- 2026-08-19 | epic-relationships shipped (7/7) | review: waves 1-4 dark-factory orchestration — epic containment/prerequisite graph schema, `tyrion epic parent/depends/waves` CLI, eligibility routing through unmet_prereqs on every start/claim/suggest path, tree + waiting state everywhere (CLI and web), the four planning skills (shape/orient/plan/complete-epic) wired to propose and read the tree; wave 4's two lanes each closed with a bookkeeping gap (pre-push gate / criteria checkboxes recorded post-hoc by the orchestrator from evidence already on the ledger, not fabricated) | unlocks: test-seq, web-epics-multitab, tyrion-polish, dark-factory-mode, spike-visibility (prerequisite now satisfied, not newly spawned)
